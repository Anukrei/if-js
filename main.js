...
function stringMatch(strArr, search) {
  ...
  return strArr.filter((item) =>
    Object.values(item).some((it) => it.toUpperCase().trim().includes(search))
  );
}
...
for (let i = 0; i < arrСhoice.length; i++) {
  if (arrСhoice[i] > 5 && arrСhoice[i] < 10) {
    console.log(
      "Элемент массива rrСhoice больше 5 но меньше 10:",
      `${arrСhoice[i]}`
    );
  }
}
...
function isPolindrome(str) {
  for (
    let leftItem = 0, rightItem = str.length - 1;
    leftItem < str.length;
    leftItem++, rightItem--
  ) {
...
for (let i = 0; i < СolorElement.length; i++) {
  // Цикл по найденному в документе.
...
function func() {
  // Функция, которую мы записали в метод addEventListener
  ...
  return function () {
    ...
    if (color === colorsArr.length) {
      // Проверка, на длину массива. Если пробежались по всем цветам из массива. (true)
